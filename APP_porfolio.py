...
if selected=="Situación actual":
    ...
    c1.metric("Posición inicial", round(metrics_df["Posición_ini"].sum(),2))
    c2.metric("Posición actual", round(metrics_df["Posición"].sum(),2))
    c3.metric("Variación", round(metrics_df["Var"].sum(),2))
    ...
    left, right = st.columns([1,1])
    with left:
        plot_donut_cartera(metrics_df_final[["categoria", "Posición"]].groupby("categoria").sum().reset_index(),
                    category_col="categoria",
                    value_col="Posición",
                    title="Fondos por Categoría")
    with right:
        plot_donut_cartera(metrics_df_final[["categoria_riesgo", "Posición"]],
        category_col="categoria_riesgo",
        value_col="Posición",
        title="Posición según riesgo (volatilidad anualizada)")
...
if selected=="Renta Variable":
    ...
    with center:
        plot_donut_cartera(metrics_df_final[metrics_df_final["categoria"]=="Renta variable"][["nombre_fondo", "Posición"]],
            category_col="nombre_fondo",
            value_col="Posición",
...
meta = (
    metrics_df_final[["isin", "nombre_fondo", "categoria"]]
    .dropna(subset=["isin", "nombre_fondo", "categoria"])
    .drop_duplicates("isin")
)
...
for cat in orden_categorias:
    fondos_cat = meta[meta["categoria"] == cat]
    if fondos_cat.empty:
        continue

    st.markdown(f"### {cat}")

    filas = [fondos_cat.iloc[i:i+4] for i in range(0, len(fondos_cat), 4)]
    ...
    for fila in filas:
        ...
        for col, (_, r) in zip(cols, fila.iterrows()):
            ...
            if isin not in df_final.columns:
                continue

            s = df_final[isin].dropna()
            if len(s) < 2:
                continue

            fecha = s.index[-1].strftime("%d/%m/%Y")
            variacion = s.iloc[-1] / s.iloc[-2] - 1

            with col:
                st.metric(
                    label=r["nombre_fondo"],
                    value=fecha,
                    delta=f"{variacion:.2%}"
                )
